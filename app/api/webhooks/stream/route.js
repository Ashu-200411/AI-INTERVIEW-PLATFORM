...
import { db } from "@/lib/prisma";
  ...
  let body;
   try {
  console.log(
    "[stream-webhook] Content-Encoding:",
    request.headers.get("content-encoding")
  );

  console.log(
    "[stream-webhook] Content-Type:",
  ...
  if (contentEncoding === "gzip") {
    ...
    console.log(
      `[stream-webhook] GZIP payload preview:\n${decompressed.slice(0, 500)}`
    );
    ...
    console.log(
      `[stream-webhook] Raw payload preview:\n${rawBody.slice(0, 500)}`
    );
    ...
    console.error("[stream-webhook] Failed to parse webhook payload:", e);
